...
function tryParseDroneHubTaskLiteral(text: string): DroneHubTask[] | null {
  const parser = new TaskLiteralParser(text);
...
function tryParseDroneHubTaskLiteralAt(text: string, startIndex: number): TaskRange | null {
  const parser = new TaskLiteralParser(text, startIndex);
  const parsed = parser.parseRoot();
  if (!parsed) return null;
  const tasks = normalizeDroneHubTaskList(parsed.value);
...
function hasOverlap(ranges: TaskRange[], start: number, end: number): boolean {
...
export function extractDroneHubTasksFromAgentMessage(textRaw: string): {
  ...
  if (ranges.length === 0) return { cleanedText: text.trim(), tasks: [] };

  ranges.sort((left, right) => left.start - right.start);
  const tasks = ranges.flatMap((range) => range.tasks);
  let cursor = 0;
  let cleaned = '';
  ...
  return {
    cleanedText: cleanupMessageAfterTaskRemoval(cleaned),
    tasks,
  };
}
