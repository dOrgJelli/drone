...
function cleanupMessageAfterLiteralRemoval(text: string): string {
  return text
    .replace(/[ \t]+\n/g, '\n')
    .replace(/\n{3,}/g, '\n\n')
    .trim();
...
function extractStructuredMessageItems<T>(opts: {
  text: string;
